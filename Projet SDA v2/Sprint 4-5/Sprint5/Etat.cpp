...
bool but(const Etat& e) {
	...
	Tableau2D blablo;
	initialiser(blablo, e.d.nbCol, e.d.nbLig);
	for (unsigned int i = 0; i < e.d.nbCol - 1; ++i) { // boucle de décalage a gauche

		for (unsigned int j = 0; j < blablo.nbLig; ++j) { // boucle des colonnes

			for (unsigned int h = 0; h < blablo.nbCol; ++h) { // boucle des lignes
				blablo.tab[j][h] = tab2D.tab[(j + 1 + i) % e.d.nbCol][h];
			}
		}
		if (compareEtatFinaux(e.d, blablo) == true) {
			detruire(tab2D);
			detruire(blablo);
			return true;
		}
...
unsigned int heuristique(Etat& e) {
	...
	Tableau2D blablo; // tableau etat finale (les lignes et les colonnes sont inversé)
	initialiser(blablo, e.d.nbCol, e.d.nbLig);
